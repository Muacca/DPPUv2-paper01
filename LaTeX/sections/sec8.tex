...
Varying $\eta$ from negative to positive produces a transition of 
Type I $\to$ Type II $\to$ Type III.
...
Exhibits behaviors characteristic of negative curvature. For $\theta_{\NY} = 0$,
the entire region is Type III (unstable) same as $\Tthree$.
For $\theta_{\NY} > 0$, Type I regions appear in $\eta < 0$, but stability is
inhibited near the origin ("leakage" of Type III), This residual instability
suggests a negative curvature-related suppression of stabilization.

\subsubsection{Geometric origin of phase structure}

The phase structure is determined by the competition between the geometric coefficient 
$C$ and the torsion coefficient $B$.

\paragraph{Decisive Role of Spatial Curvature:}
The spatial curvature determines the baseline of $C$:
\begin{itemize}
  \item $\Sthree$ (Positive Curvature): Contributes $-36$ to $C$, allowing $C < 0$ and 
        geometric stability (Type II) even without torsion.
  \item $\Nilthree$ (Negative Curvature): Contributes $+3$ to $C$, enforcing $C > 0$ 
        and inhibiting stability near the origin ("negative curvature penalty").
  \item $\Tthree$ (Zero Curvature): $C \ge 0$, stability is strictly torsion-driven.
\end{itemize}

\paragraph{Symmetry Breaking:}
$\eta \to -\eta$ symmetry is broken for all topologies when $\theta_{\NY} > 0$ 
due to the linear dependence $B \propto \eta$. Stability is generally favored 
in the $\eta < 0$ region where $B$ contributes negatively to the potential.

\subsubsection{Uniformity of Construction Variants}

We found that all three NY variants (FULL, TT, REE) share the same qualitative
phase structure, driven by the linear dependence $B \propto \eta$.
This suggest that the stabilization mechanism is robust in the NY term.
